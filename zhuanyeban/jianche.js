...
const testImpedance = document.getElementById('testImpedance');
const selectBadLead = document.getElementById('selectBadLead');
const impedanceCanvas = document.getElementById('impedanceCanvas');
const ctx = impedanceCanvas.getContext('2d');

// 模拟 32 通道的阻抗数据
let currentImpedanceData = new Array(32).fill(0).map(() => Math.random() * 30);
...
function drawChannel(x, y, impedance) {
  ...
  ctx.fillText(impedance.toFixed(1), x, y);
}
...
function updateImpedanceData() {
  targetImpedanceData = new Array(32).fill(0).map(() => Math.random() * 30);
  startTime = null;
  requestAnimationFrame(animate);
}
...
testImpedance.addEventListener('click', () => {
  targetImpedanceData = new Array(32).fill(0).map(() => Math.random() * 30);
  startTime = null;
  requestAnimationFrame(animate);
  console.log('阻抗测试');
...
selectBadLead.addEventListener('click', () => {
  console.log('坏导选择');
});

// 启动动态更新
intervalId = setInterval(updateImpedanceData, 3000);
